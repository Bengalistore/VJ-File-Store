...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('ᴄʜᴀɴɴᴇʟ', url='https://t.me/ainfilm')
        ],[
...
# Ask Doubt on telegram @KingVJ01
    ...
    try:
        ...
        title = '@ainfilm  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
        size=get_size(file.file_size)
        f_caption = f"<code>{caption}</code>"
